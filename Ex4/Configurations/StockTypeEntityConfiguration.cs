...
{
    class StockTypeEntityConfiguration : IEntityTypeConfiguration<StockType>
    {
        public void Configure(EntityTypeBuilder<StockType> builder)
        {
            builder.ToTable("StockType");
            builder.HasKey(s => s.StockTypeID);
            builder.Property(p => p.StockTypeName)
                    .HasMaxLength(20);
            //builder.HasData(
